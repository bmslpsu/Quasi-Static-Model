...
cut_time_force = struct([]);
cut_time_torque = struct([]);
j = 1;
for i = 1:length(Fly_Master)
    if Fly_Master(i).Attributes == "Pre-Cut"
        FlyNum = Fly_Master(i).Fly_Num;
        
        % Find the matching Post-Cut entry
        post_idx = find([Fly_Master.Fly_Num] == FlyNum & ...
                        strcmp({Fly_Master.Attributes}, "Post-Cut"), 1);

        if isempty(post_idx)
            warning("No Post-Cut entry found for Fly #%d. Skipping.", FlyNum);
            continue;
        end
        ...
        % Set up structure
        cut_time_force(j).Fly_Num = FlyNum;
        cut_time_torque(j).Fly_Num = FlyNum;

        % Determine which wing is cut
        if Fly_Master(post_idx).Chord_Cut_LH == 100
            cut_time_force(j).cut_percentage = Fly_Master(post_idx).chord_cut_RH;
            cut_time_torque(j).cut_percentage = Fly_Master(post_idx).chord_cut_RH;

            % Pre-Cut (LH is healthy)
            Phi = Fly_Master(i).Fly.Kinematics.RH.phi;
            [~, ~, ~, cut_time_force(j).x_last_pre, cut_time_force(j).y_last_pre, cut_time_force(j).z_last_pre, ~, cut_time_force(j).all_last] = ...
                force_period_Mean(i, Phi, Fly_Master(i).Fly.Dynamics.Frame_Body.LH.Force_Total, weight);
            [~, ~, ~, cut_time_torque(j).x_last_pre, cut_time_torque(j).y_last_pre, cut_time_torque(j).z_last_pre, ~, cut_time_torque(j).all_last] = ...
                torque_period_Mean(i, Phi, Fly_Master(i).Fly.Dynamics.Frame_Body.LH.Torque_Total, weight);

            % Post-Cut
            Phi = Fly_Master(post_idx).Fly.Kinematics.RH.phi;
            [cut_time_force(j).x_last_post, cut_time_force(j).y_last_post, cut_time_force(j).z_last_post,~, ~, ~, cut_time_force(j).all_first, ~] = ...
                force_period_Mean(post_idx, Phi, Fly_Master(post_idx).Fly.Dynamics.Frame_Body.LH.Force_Total, weight);
            [cut_time_torque(j).x_last_post, cut_time_torque(j).y_last_post, cut_time_torque(j).z_last_post,~, ~, ~, cut_time_torque(j).all_first, ~] = ...
                torque_period_Mean(post_idx, Phi, Fly_Master(post_idx).Fly.Dynamics.Frame_Body.LH.Torque_Total, weight);
        else
            cut_time_force(j).cut_percentage = Fly_Master(post_idx).Chord_Cut_LH;
            cut_time_torque(j).cut_percentage = Fly_Master(post_idx).Chord_Cut_LH;

            % Pre-Cut (RH is healthy)
            Phi = Fly_Master(i).Fly.Kinematics.LH.phi;
            [~, ~, ~, cut_time_force(j).x_last_pre, cut_time_force(j).y_last_pre, cut_time_force(j).z_last_pre, ~, cut_time_force(j).all_last] = ...
                force_period_Mean(i, Phi, Fly_Master(i).Fly.Dynamics.Frame_Body.RH.Force_Total, weight);
            [~, ~, ~, cut_time_torque(j).x_last_pre, cut_time_torque(j).y_last_pre, cut_time_torque(j).z_last_pre, ~, cut_time_torque(j).all_last] = ...
                torque_period_Mean(i, Phi, Fly_Master(i).Fly.Dynamics.Frame_Body.RH.Torque_Total, weight);

            % Post-Cut
            Phi = Fly_Master(post_idx).Fly.Kinematics.LH.phi;
            [cut_time_force(j).x_last_post, cut_time_force(j).y_last_post, cut_time_force(j).z_last_post,~, ~, ~, cut_time_force(j).all_first, ~] = ...
                force_period_Mean(post_idx, Phi, Fly_Master(post_idx).Fly.Dynamics.Frame_Body.RH.Force_Total, weight);
            [cut_time_torque(j).x_last_post, cut_time_torque(j).y_last_post, cut_time_torque(j).z_last_post,~, ~, ~, cut_time_torque(j).all_first, ~] = ...
                torque_period_Mean(post_idx, Phi, Fly_Master(post_idx).Fly.Dynamics.Frame_Body.RH.Torque_Total, weight);
        end

        % Validate fields — if all_last is missing or NaN, skip this entry
        if ~isfield(cut_time_force(j), 'all_last') || ~isfield(cut_time_force(j).all_last, 'x_first') || ...
           any(isnan(cut_time_force(j).all_last.x_first))
            warning("Skipping Fly #%d due to missing or invalid 'all_last'.", FlyNum);
            continue;
        end

        % Compute period info (optional but useful)
        [~, Peaks] = findpeaks(Phi);
        if length(Peaks) >= 2
            ...
            cut_time_force(j).Percentage_after_peak = NaN;
            cut_time_torque(j).Percentage_after_peak = NaN;
        end

        j = j + 1;
    end
end
% If no valid entries found, return empty structs
if j == 1
    warning("No valid Pre-Cut/Post-Cut fly pairs were found.");
    cut_time_force = struct([]);
    cut_time_torque = struct([]);
end
end
